...
class ResnetPixelCNNVAE(nn.Module):
    def __init__(self, z_dim=128, channels=3, beta=1, device=None, loss_type='mse', output_size=28):
        super().__init__()
        self.device = device
        self.encoder = ResNetVariationalEncoder(z_dim, channels, output_size)
        self.decoder = PixelCNNDecoderV2(z_dim, output_size)
